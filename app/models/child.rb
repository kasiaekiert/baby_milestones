class Child < ApplicationRecord
  ...
  def reload_assignments!
    logger.info("Reloading assignments for #{child.name.inspect} (#{child.id})")
    ...
    query.each do |milestone|
      logger.info("Assigning #{milestone.title.inspect} (#{milestone.id}) to #{child.name.inspect} (#{child.id})")
